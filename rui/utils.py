...
class TextVectorizer:
    def __init__(self, max_tokens=20000, output_mode="int", output_sequence_length=None,
                 ngrams=None, standardize=None):
    ...
    def adapt(self, texts):
        ...
        most_common = token_counter.most_common(
            self.max_tokens - 2 if self.max_tokens else None
        )
        # 0 = padding, 1 = unknown
    ...
    def load(cls, path, standardize=None):
        # BUG-FIX: original code passed the path string directly to pickle.load()
        with open(path, 'rb') as f:
            data = pickle.load(f)
        vectorizer = cls(
            max_tokens=data['max_tokens'],
            output_sequence_length=data['output_sequence_length'],
            standardize=standardize if data.get('has_standardizer') else None,
        )
